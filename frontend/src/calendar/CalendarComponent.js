...
class CalendarComponent extends React.Component {
  constructor(props) {
    ...
    dispatch(
      restActions.restGet('/speakers', restConstants.GET_SPEAKER_REQUEST)
    );
  ...
  commitDeletedAppointment() {
    ...
    const { dispatch, appointments } = this.props;
    const deleted = appointments.filter(
      appointment => appointment.id === deletedAppointmentId
    )[0];
    dispatch(
      AppointmentActions.deleteAppointment(deleted.event_id, deleted.id)
    );
  ...
  onAddedAppointmentChange(addedAppointment) {
    addedAppointment.startDate = moment
      .unix(addedAppointment.startDate.getTime() / 1000)
      .format('YYYY-MM-DD HH:mm');
    addedAppointment.endDate = moment
      .unix(addedAppointment.endDate.getTime() / 1000)
      .format('YYYY-MM-DD HH:mm');
  ...
  commitChanges({ added, changed, deleted }) {
    ...
    if (added) {
      ...
      dispatch(
        AppointmentActions.postAppointment(
          appointment.event,
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const { currentPath } = this.state;
    if (
      newAppointment !== prevProps.newAppointment ||
          ...
          <Paper>
            <Scheduler
              data={
                currentPath === -1
                  ? appointments
                  : filterData(appointments, currentPath)
              }
            >
              <ViewState
...
const filterData = (data, pathId) =>
  data.filter(event => event.path_id === pathId);
