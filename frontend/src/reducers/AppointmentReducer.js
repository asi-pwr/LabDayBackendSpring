    ...
    case restConstants.GET_EVENT_REQUEST:
      ...
      state.timetables.forEach(function(appointment, index) {
        ...
        newAppointments[index] = {
          ...appointment,
          ...action.data.filter(event => event.id === event_id)[0]
        };
        newAppointments[index].id = appointmentId;
        newAppointments[index].title = newAppointments[index].name;
        newAppointments[index].startDate = moment
          .unix(newAppointments[index].time_start)
          .format('YYYY-MM-DD HH:mm');
        newAppointments[index].endDate = moment
          .unix(newAppointments[index].time_end)
          .format('YYYY-MM-DD HH:mm');
      });
      return {
