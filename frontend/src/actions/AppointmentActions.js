...
function getAppointments() {
  return dispatch => {
    ...
    axiosInstance.get('/timetables').then(response => {
      dispatch({
        status: response.status,
...
function postAppointment(event, timetable) {
  return dispatch => {
    axiosInstance.post('/events', JSON.stringify(event)).then(response => {
      dispatch({
        status: response.status,
      ...
      axiosInstance
        .post('/timetables', JSON.stringify(timetable))
        .then(response => {
          dispatch({
            status: response.status,
...
function deleteAppointment(eventId, timetableId) {
  return dispatch => {
    axiosInstance.delete('/events/' + eventId).then(response => {
      dispatch({
        status: response.status,
        deletedItem: eventId,
    ...
    axiosInstance.delete('/timetables/' + timetableId).then(response => {
      dispatch({
        status: response.status,
